...
class PokerOracle:
    ...
    def hand_classifier(self, card_set: list[Card]) -> tuple[str, int]: 
        ...
        if not num_cards == 5:
            ...
            for subset in subsets:
                subset_classification, subset_ranking = self.hand_classifier(subset)
            ...
            if self.is_flush(card_set):
                if self.is_straight(card_set):
                    if self.get_highest_card(card_set).get_rank() == 14: # TODO: Make this a constant? Or use something from the card_deck class?
                        hand = "royal_flush"
                        return hand, self.hand_rankings[hand] 
                    hand = "straight_flush"
                    return hand, self.hand_rankings[hand]
                hand = "flush"
                return hand, self.hand_rankings[hand]
    ...
    def get_highest_card(self, card_set: list[Card]) -> Card:
        return sorted(card_set, key=lambda card: card.get_rank())[-1]
    ...
    def is_full_house(self, card_set: list[Card]):
        num_each_rank = self.count_ranks(card_set)
        return 2 in num_each_rank and 3 in num_each_rank
    
    def is_four_of_a_kind(self, card_set: list[Card]):
        num_each_rank = self.count_ranks(card_set)
        return 4 in num_each_rank
            
    def is_three_of_a_kind(self, card_set: list[Card]):
        # Assumes full house is checked before this
        num_each_rank = self.count_ranks(card_set)
